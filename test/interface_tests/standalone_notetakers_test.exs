defmodule ExNylas.StandaloneNotetakersTest do
  use ExUnit.Case, async: true
  alias ExNylas.{StandaloneNotetakers, Response, Notetaker}
  import Plug.Conn
  ...
  test "cancel/2 returns success on success", %{bypass: bypass} do
    Bypass.expect_once(bypass, "DELETE", "/v3/notetakers/notetaker-id/cancel", fn conn ->
      conn
  ...
  test "cancel/2 returns error on failure", %{bypass: bypass} do
    ...
    assert {:error, %ExNylas.Response{status: :not_found}} =
      StandaloneNotetakers.cancel(default_connection(bypass), "notetaker-id")
  end
  ...
  test "leave/2 returns success on success", %{bypass: bypass} do
    Bypass.expect_once(bypass, "POST", "/v3/notetakers/notetaker-id/leave", fn conn ->
      conn
      |> put_resp_content_type("application/json")
      |> send_resp(200, ~s<{"data": {"id": "notetaker-id"}}>)
    end)
  ...
  test "media/2 returns success on success", %{bypass: bypass} do
    Bypass.expect_once(bypass, "GET", "/v3/notetakers/notetaker-id/media", fn conn ->
      conn
      |> put_resp_content_type("application/json")
