defmodule ExNylasTest.Webhooks do
  ...
  test "valid_signature? raises an error if connection struct lacks `client_secret`" do
    assert_raise RuntimeError, fn ->
      ExNylas.Webhooks.valid_signature?(%ExNylas.Connection{}, "", "")
    end
  end
  ...
  test "valid_signature? returns false if the signature does not match" do
    ...
    assert ExNylas.Webhooks.valid_signature?(conn, "", "") == false
  end
  ...
  test "valid_signature? returns true if the signature matches" do
    ...
    assert ExNylas.Webhooks.valid_signature?(conn, body, signature) == true
  end
end
