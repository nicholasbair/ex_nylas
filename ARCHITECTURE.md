...
Functions ending in `!` like `list!/2`, `find!/3`, `create!/3`, etc. raise `ExNylasError` on failure:

```elixir
# Success - returns the response directly
response = ExNylas.Messages.list!(conn)

# Failure - raises ExNylasError
# ** (ExNylasError) Request failed with status 401
```
...
Both versions are auto-generated by the `use ExNylas` macro. The bang version is a thin wrapper:
